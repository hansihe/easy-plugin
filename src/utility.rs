...
/// A type whose values can be generated by evaluating `P<Expr>`s.
pub trait ToExpr {
    /// Returns a `P<Expr>` which would produce this value if evaluated.
...
impl<'s> TransactionParser<'s> {
    ...
    pub fn get_span(&self) -> Span {
        if self.last == self.tokens.len() {
            ...
            self.tokens.get(self.last).expect("expected span").sp
        }
    }
    ...
    pub fn get_last_span(&self) -> Span {
        if self.last == self.tokens.len() {
            ...
            self.tokens.get(self.last.saturating_sub(1)).expect("expected span").sp
        }
    }
